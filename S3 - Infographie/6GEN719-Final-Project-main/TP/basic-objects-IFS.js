...
/**
 * Defines a model of a triangular base prism. The axis of the prism is the z-axis,
 * and the center is at (0,0,0).
 * @param baseLength the length of each side of the triangular base.
 * @param height the height of the prism. The prism extends from -height/2
 * to height/2 along the z-axis.
 */
function uvTriangularPrism(baseLength, height) {
   ...
   // Bottom face
   vertices.push(-halfBase, -halfBase * sqrt3 / 3, -halfHeight);
   vertices.push(halfBase, -halfBase * sqrt3 / 3, -halfHeight);
   vertices.push(0, halfBase * sqrt3 * 2 / 3, -halfHeight);
   for (var i = 0; i < 3; i++) {
       normals.push(0, 0, -1);
       texCoords.push(i / 2, i % 2);
   }
   ...
   for (var i = 0; i < 3; i++) {
       ...
       vertices.push(vertices[next * 3], vertices[next * 3 + 1], vertices[next * 3 + 2]);
       vertices.push(vertices[i * 3 + 9], vertices[i * 3 + 10], vertices[i * 3 + 11]);
       vertices.push(vertices[next * 3 + 9], vertices[next * 3 + 10], vertices[next * 3 + 11]);

       for (var j = 0; j < 4; j++) {
           normals.push(sideNormals[i][0], sideNormals[i][1], sideNormals[i][2]);
           texCoords.push(j % 2, Math.floor(j / 2));
       }
   }

   // Indices
   indices.push(0, 1, 2); // Bottom face
   indices.push(3, 4, 5); // Top face
...
/**
 * Defines a model of a triangular base prism with a right-angled triangle as the base.
 * The axis of the prism is the z-axis, and the center is at (0,0,0).
 * @param baseLength the length of the two perpendicular sides of the triangular base.
 * @param height the height of the prism. The prism extends from -height/2 to height/2 along the z-axis.
 */
function uvTriangularPrismRectangle(baseLength, height) {
   baseLength = baseLength || 1;
   height = height || 1;
   ...
   vertices.push(baseLength, 0, -halfHeight);
   vertices.push(0, baseLength, -halfHeight);
   for (var i = 0; i < 3; i++) {
       normals.push(0, 0, -1);
       texCoords.push(i / 2, i % 2);
   }
